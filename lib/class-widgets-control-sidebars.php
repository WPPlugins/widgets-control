...
class Widgets_Control_Sidebars {
	...
	public static function dynamic_sidebar_after( $index, $has_widgets ) {
		...
		if ( is_admin() && $index != 'wp_inactive_widgets' ) {
			...
			echo ' [home] [front] [single] [page] [category] [category:xyz] [has_term:term:taxonomy] [tag] [tag:xyz] [tax] [tax:taxonomy] [tax:taxonomy:term] [author] [author:xyz] [archive] [search] [404]';
			if ( defined( 'ICL_LANGUAGE_CODE' ) ) {
				echo ' [language:xyz]';
			}
			echo '<br/>';
			echo __( 'Please refer to the <a href="http://docs.itthinx.com/document/widgets-control/">Documentation</a> for details.', WIDGETS_PLUGIN_DOMAIN );
			echo '</p>';
			...
			echo '<p>';
			echo __( 'Get <a href="http://www.itthinx.com/shop/widgets-control-pro/"><strong>Widgets Control Pro</strong></a> which supports conditions based on page hierarchies, post types, roles and <a href="http://wordpress.org/plugins/groups/">groups</a>.', WIDGETS_PLUGIN_DOMAIN );
			echo '</p>';
